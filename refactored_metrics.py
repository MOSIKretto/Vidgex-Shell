"""Refactored metrics module following DRY principles and fixing memory leaks."""
...
class Metrics(BaseComponent):
    ...
    def __init__(self, **kwargs):
        ...
        self.temp_widget = self._create_singular_metric("temp", "ТЕМП", icons.temp)
        self.disk_widgets = [self._create_singular_metric("disk", "ДИСК", icons.disk) for _ in ["/"]]
        self.ram_widget = self._create_singular_metric("ram", "ОЗУ", icons.memory)
        self.cpu_widget = self._create_singular_metric("cpu", "ЦП", icons.cpu)
        
        # Get GPU info and create widgets
        try:
            gpu_info = self.provider.get_gpu_info()
            self.gpu_widgets = [self._create_singular_metric(f"gpu", "GPU", icons.gpu) for _ in gpu_info]
        except:
            self.gpu_widgets = []

        # Add all widgets to the box
        all_widgets = [self.temp_widget, self.ram_widget, self.cpu_widget] + \
                      self.disk_widgets + self.gpu_widgets
        
        for widget in all_widgets:
            if widget and hasattr(widget, 'usage'):
                widget.usage.set_sensitive(False)
            if widget and hasattr(widget, 'box'):
                self.box.add(widget.box)

        # Start update timer
        timer_id = GLib.timeout_add_seconds(2, self.update_status)
        self.add_timer(timer_id)

    def _create_singular_metric(self, metric_id: str, name: str, icon: str):
        """Create a singular metric widget."""
        usage = Scale(
            name=f"{metric_id}-usage",
            value=0.25,
            orientation='v',
            inverted=True,
            v_align='fill',
        ...
        label = Label(
            name=f"{metric_id}-label",
            markup=icon,
        )

        box = Box(
            name=f"{metric_id}-box",
            orientation='v',
            spacing=8,
            children=[
                usage,
        ...
        box.set_tooltip_markup(f"{icon} {name}")
        
        return type('SingularMetric', (), {
            'usage': usage,
            'label': label,
            'box': box
        })()
    ...
    def update_status(self):
        ...
        if self.temp_widget:
            self.temp_widget.usage.value = min(temperature / 150.0, 1.0)
        for i, disk in enumerate(self.disk_widgets):
            if i < len(disks):
                disk.usage.value = disks[i] / 100.0
        if self.ram_widget:
            self.ram_widget.usage.value = mem / 100.0
        if self.cpu_widget:
            self.cpu_widget.usage.value = cpu / 100.0
