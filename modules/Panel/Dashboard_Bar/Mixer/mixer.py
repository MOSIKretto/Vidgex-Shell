...
class MixerSlider(Scale):
    def __init__(self, stream, **kwargs):
        ...
        self._value_changed_id = self.connect("value-changed", self.on_value_changed)
    ...
    def destroy(self):
        if hasattr(self, 'stream') and self.stream and hasattr(self, '_stream_changed_id'):
            self.stream.disconnect(self._stream_changed_id)
        
        if hasattr(self, '_value_changed_id'):
        ...
        for child in self.content_box.get_children():
            if isinstance(child, Box):
                for widget in child.get_children():
                    if isinstance(widget, MixerSlider):
                        widget.destroy()
...
class Mixer(Box):
    ...
    def update_mixer(self):
        ...
        if self.audio.speaker: outputs.append(self.audio.speaker)
        outputs.extend(self.audio.applications)

        if self.audio.microphone: inputs.append(self.audio.microphone)
