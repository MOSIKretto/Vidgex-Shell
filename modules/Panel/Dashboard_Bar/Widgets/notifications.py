...
_pixbuf_cache = {}
_pixbuf_cache_size = 0
_MAX_PIXBUF_CACHE_SIZE = 15 * 1024 * 1024 
_pixbuf_cache_order = [] 

def _cleanup_pixbuf_cache():
    global _pixbuf_cache, _pixbuf_cache_size, _pixbuf_cache_order
    _pixbuf_cache.clear()
    _pixbuf_cache_size = 0
    _pixbuf_cache_order.clear()

atexit.register(_cleanup_pixbuf_cache)


def cache_notification_pixbuf(notification_box):
    global _pixbuf_cache, _pixbuf_cache_size, _MAX_PIXBUF_CACHE_SIZE, _pixbuf_cache_order
    
    notification = notification_box.notification
    if notification.image_pixbuf:
        os.makedirs(PERSISTENT_DIR, exist_ok=True)
        cache_key = f"notification_{notification_box.uuid}"
        cache_file = os.path.join(PERSISTENT_DIR, f"{cache_key}.png")
        
        try:
            scaled = notification.image_pixbuf.scale_simple(
                48, 48, GdkPixbuf.InterpType.BILINEAR
            )
            pixbuf_size = scaled.get_width() * scaled.get_height() * 4
            
            while _pixbuf_cache_size + pixbuf_size > _MAX_PIXBUF_CACHE_SIZE and _pixbuf_cache_order:
                oldest_key = _pixbuf_cache_order.pop(0)
                if oldest_key in _pixbuf_cache:
                    old_pb = _pixbuf_cache[oldest_key]
                    old_size = old_pb.get_width() * old_pb.get_height() * 4
                    _pixbuf_cache_size -= old_size
                    del _pixbuf_cache[oldest_key]
                
                old_file = os.path.join(PERSISTENT_DIR, f"{oldest_key}.png")
                if os.path.exists(old_file):
                    os.remove(old_file)
                
            
            if cache_key in _pixbuf_cache_order:
                _pixbuf_cache_order.remove(cache_key)
                _pixbuf_cache_size -= (_pixbuf_cache[cache_key].get_width() * _pixbuf_cache[cache_key].get_height() * 4)
            
            scaled.savev(cache_file, "png", [], [])
            _pixbuf_cache[cache_key] = scaled
            _pixbuf_cache_size += pixbuf_size
            _pixbuf_cache_order.append(cache_key)
...
def load_scaled_pixbuf(notification_box, width, height):
    ...
    if notification.image_pixbuf:
        if (notification.image_pixbuf.get_width() != width or notification.image_pixbuf.get_height() != height):
...
class NotificationHistory(Box):
    ...
    def add_notification(self, notification_box):
        if len(self.containers) >= 50:
            ...
            if (hasattr(oldest_container, "notification_box") and hasattr(oldest_container.notification_box, "cached_image_path")
                and oldest_container.notification_box.cached_image_path and os.path.exists(oldest_container.notification_box.cached_image_path)):
...
class NotificationPopup(Window):
    def __init__(self, **kwargs):
        ...
        if y_pos == "top":
            anchor = f"{x_pos} top"
            revealer_type = "slide-down"
        else:
            anchor = f"{x_pos} bottom"
            revealer_type = "slide-up"
        ...
        self.notification_container = NotificationContainer(
            notification_history_instance=self.notification_history,
            revealer_transition_type=revealer_type,
        )
