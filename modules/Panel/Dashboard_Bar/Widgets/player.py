...
def _cleanup_temp_artwork_files():
    for temp_file in _temp_artwork_files:
        if os.path.exists(temp_file):
            os.remove(temp_file)
    _temp_artwork_files.clear()
...
class PlayerBox(Box):
    ...
    def _apply_mpris_properties(self):
        ...
        if mp.arturl:
            ...
            if parsed.scheme == "file":
                ...
                self._download_artwork(mp.arturl)
            else:
        ...
        def worker(user_data):
            ...
            try:
                ...
                if len(data) > 5 * 1024 * 1024:
                    ...
                    _temp_artwork_files.append(local_arturl)
            except Exception:
                local_arturl = os.path.expanduser("~/.current.wall")
    ...
    def destroy(self):
        ...
        if hasattr(self, '_temp_artwork_files'):
...
class PlayerSmall(CenterBox):
    def __init__(self):
        ...
        if players:
            ...
            self._apply_mpris_properties()
            self.mpris_player.connect("changed", self._apply_mpris_properties())
        else:
            self._apply_mpris_properties()
    ...
    def _on_icon_button_press(self, widget, event):
        ...
        if event.type == Gdk.EventType.BUTTON_PRESS:
            players = self.mpris_manager.playersc
            if not players:
                return True
        ...
        if event.type == Gdk.EventType.BUTTON_PRESS:
            if event.button == 1:
                if self.mpris_player:
                    ...
                    if not hasattr(self, '_restore_timer'):
