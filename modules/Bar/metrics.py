...
class MetricsProvider:
    ...
    def _tick(self):
        ...
        try:
            st = os.statvfs('/')
            t, fr = st.f_blocks * st.f_frsize, st.f_bfree * st.f_frsize
...
class Metrics(Box):
    ...
    def _upd(self):
        if not _prov:
            return
        self.temp.usage.value = min(_prov.temp * 0.00666667, 1.0)
        self.ram.usage.value = _prov.mem * 0.01
        self.cpu.usage.value = _prov.cpu * 0.01
        for i, d in enumerate(self.disk):
            if i < len(_prov.disk):
                d.usage.value = _prov.disk[i] * 0.01
        for i, g in enumerate(self.gpu):
            if i < len(_prov.gpu):
                g.usage.value = _prov.gpu[i] * 0.01
...
class MetricsSmall(Button):
    ...
    def _upd(self):
        if not _prov:
            return
        self.temp.circle.set_value(min(_prov.temp * 0.00666667, 1.0))
        self.temp.level.set_label(f'{int(_prov.temp + 0.5)}°C')
        self.cpu.circle.set_value(_prov.cpu * 0.01)
        self.cpu.level.set_label(f'{int(_prov.cpu)}%')
        self.ram.circle.set_value(_prov.mem * 0.01)
        self.ram.level.set_label(f'{int(_prov.mem)}%')
        for i, d in enumerate(self.disk):
            if i < len(_prov.disk):
                d.circle.set_value(_prov.disk[i] * 0.01)
                d.level.set_label(f'{int(_prov.disk[i])}%')
        for i, g in enumerate(self.gpu):
            if i < len(_prov.gpu):
                g.circle.set_value(_prov.gpu[i] * 0.01)
                g.level.set_label(f'{int(_prov.gpu[i])}%')
...
class Battery(Box):
    ...
    def set_auto_mode(self, en):
        self.auto, self.manual = en, False
        if en and _prov:
...
class NetworkApplet(Button):
    ...
    def _upd(self):
        if not _prov:
            return
        self.dl.set_markup(self._fmt(_prov.net_dl))
        self.ul.set_markup(self._fmt(_prov.net_ul))
