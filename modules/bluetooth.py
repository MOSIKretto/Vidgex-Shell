...
class BluetoothDeviceSlot(CenterBox):
    ...
    def on_changed(self, *_):
        ...
        if self.device.connecting: self.connect_button.set_label("Connecting..." if not self.device.connecting else "...")
        else: self.connect_button.set_label("Connect" if not self.device.connected else "Disconnect")

        if self.device.connected: self.connect_button.add_style_class("connected")
        else:self.connect_button.remove_style_class("connected")
...
class BluetoothConnections(Box):
    def __init__(self, **kwargs):
        ...
        self.other_box = Box(spacing=2, orientation="vertical")
        
        self.connected_label = Label(name="bluetooth-section", label="Connected")
        self.accessible_label = Label(name="bluetooth-section", label="Accessible")

        content_box = Box(spacing=4, orientation="vertical")
        content_box.add(self.connected_label)
        content_box.add(self.connected_box)
        content_box.add(self.accessible_label)
        content_box.add(self.other_box)
    ...
    def on_device_added(self, client: BluetoothClient, address: str):
        ...
        self.device_slots[address] = slot
        
        slot.device.connect("changed", lambda *_: self.update_device_position(address))
        
        self.update_device_position(address)

    def update_device_position(self, address: str):
        if address not in self.device_slots:
            return
            
        slot = self.device_slots[address]
        device = slot.device
        
        if slot.get_parent() == self.connected_box:
            self.connected_box.remove(slot)
        elif slot.get_parent() == self.other_box:
            self.other_box.remove(slot)
        
        if device.connected:
            self.connected_box.add(slot)
        else:
            self.other_box.add(slot)
    ...
    def update_scan_label(self):
        if self.client.scanning:
            ...
            self.scan_button.set_tooltip_text("Stop scanning")
        else:
            self.scan_label.remove_style_class("scanning")
            self.scan_button.remove_style_class("scanning")
            self.scan_button.set_tooltip_text("Scan Bluetooth")