...
class MetricsProvider:
    ...
    def destroy(self):
        """Очистка ресурсов MetricsProvider"""
        ...
        # Остановить UPowerManager если есть метод destroy
...
class NetworkApplet(Button):
    ...
    def _get_primary_device(self):
        if (self.network_client.ethernet_device and 
            ...
            return "ethernet"
        elif (self.network_client.wifi_device and 
              hasattr(self.network_client.wifi_device, 'enabled') and 
              self.network_client.wifi_device.enabled and
