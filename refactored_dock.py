"""Refactored dock module following DRY principles."""
...
def createSurfaceFromWidget(widget):
    """Create a surface from a widget."""
...
class Dock(BaseComponent):
    """Refactored dock component following DRY principles."""
    ...
    def _setup_event_handlers(self):
        ...
        for event, handler in events:
            handler_id = self.conn.connect(event, handler)
            self.add_signal_handler(self.conn, handler_id)

        if self.conn.ready:
            self._on_ready()
        else:
            ready_handler = self.conn.connect("event::ready", lambda *args: self._on_ready())
            self.add_signal_handler(self.conn, ready_handler)
    ...
    def _perform_occlusion_check(self):
        ...
        self._occlusion_pending = False
        if self._alive or self.integrated_mode: return False
            ...
            for monitor in monitors:
                if monitor.get("id") == self.monitor_id:
                    ...
                    if abs(self.icon_size - calculated_size) > 2:
                        ...
                        self._schedule_full_update()  # Rebuild icons with new size
    ...
    def _rebuild_dock_icons(self, clients: List[Dict[str, Any]]) -> None:
        ...
        for class_name, instances in running_windows.items():
            ...
            identifier = {
                ...
                "display_name": app.display_name if app else "",
                "window_class": app.window_class if app else "",
                "executable": app.executable if app else "",
                "command_line": app.command_line if app else ""
    ...
    def create_button(self, app_identifier: Union[Dict[str, Any], str], instances: List[Dict[str, Any]], window_class: str):
        ...
        # Use self.icon_size, which is computed mathematically
        ...
        content = FabricBox(name="dock-icon", orientation="v", h_align="center", children=[Image(pixbuf=icon_pixbuf)])
    ...
    def _setup_btn_dnd(self, btn):
        """Setup drag and drop for button."""
